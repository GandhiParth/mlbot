Here's the clean, complete prompt — save the whole thing as `classifier_prompt.md`:
...
Your response must be **only a valid JSON object** that can be parsed directly by `json.loads()`. No text before the JSON, no text after, no markdown code fences, no explanation.

Use this exact schema. All fields must be present. Use `null` for fields that don't apply (e.g., `anchor_ma` when state is `NO_SETUP`).

```json
{
  "timeframe": "1D",
  "state": "SETUP_READY",
  "label": "STANDARD_PULLBACK",
  "anchor_ma": "21 EMA",
  "chart_cleanliness": "Clean",
  "confidence": "High",
  "reasoning": {
    "cleanliness": "Brief assessment of how well this stock respects MAs historically.",
    "regime": "What you observed about the MA stack and trend structure. Note closing behavior relative to key MAs.",
    "thrust": "Where the recent upmove came from, approximate size in typical bar ranges.",
    "pause": "What has happened since the peak — pullback, contraction, or neither.",
    "anchor_identification": "Why you picked this MA, where the pullback low landed, what the close looked like at that point. Use null if state is not SETUP_READY.",
    "health_check": "Pass or fail with reasoning. Explicitly note closing behavior at key MAs."
  },
  "notes": "Anything ambiguous, borderline, or worth flagging. Empty string if nothing.",
  "above_200_sma": true
}
```

### Field constraints (must follow exactly)

- **timeframe** (string): exactly as shown on the chart, e.g. `"1D"`, `"1W"`, `"4H"`, `"1H"`, `"15m"`.
- **state** (string): one of `"NO_SETUP"`, `"WATCH"`, `"SETUP_READY"`.
...
- **anchor_ma** (string or null): one of `"9 EMA"`, `"21 EMA"`, `"50 SMA"`, or `null` if not applicable.
- **chart_cleanliness** (string): one of `"Clean"`, `"Moderately Messy"`, `"Very Messy"`.
- **confidence** (string): one of `"High"`, `"Medium"`, `"Low"`.
- **reasoning** (object): all six sub-fields must be present as strings. Use `null` for `anchor_identification` if state is not SETUP_READY. All other reasoning fields must always contain text — even for NO_SETUP, explain why.
- **notes** (string): empty string `""` if nothing to flag.
- **above_200_sma** (boolean or null): `true`, `false`, or `null` if the 200 SMA isn't visible on the chart.

### Critical rules

1. Output JSON only — no text before, no text after, no code fences.
2. Use exact string values from the enums above — `"21 EMA"` not `"21EMA"`, `"SETUP_READY"` not `"Setup Ready"`.
3. All field names must match exactly (snake_case as shown).
4. The JSON must be valid — no trailing commas, no comments, no unescaped newlines inside string values.

---

That's the full clean version. Want me to save it as `classifier_prompt.md` to download?